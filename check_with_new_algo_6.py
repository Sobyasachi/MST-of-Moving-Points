...
class point:
    ...
    def old_distance(self, another_point):
        return math.sqrt((self.oldx - another_point.oldx) ** 2 + (self.oldy - another_point.oldy) ** 2)
...
for iterations in tqdm(range(10000)):
    ...
    for i in range(len(all_points)):
        for j in range(len(all_points)):
            if i != j:
                H.add_edge(all_points[i], all_points[j],
                           weight=max_weight_matrix[i][j] + (all_points[i].old_distance(all_points[j]) / 1000000000))
    ...
    if max_of_change < max_cost / mmst_cost:
        ...
        print(max_of_change)
    with open('record6.csv', 'a') as csvfile:
        csvwriter = csv.writer(csvfile)
        row = [str(x) for x in all_points]
        row.append(str(random_point_name))
        row.extend([mmst_cost, max_cost, max_cost / mmst_cost,max_of_change])
